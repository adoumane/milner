...
\newcommand{\norm}[1]{\lVert #1 \rVert}
\newcommand{\ctrct}[1]{\llbracket #1 \rrbracket}
...
\begin{definition}
The \emph{normalization} of a network~$C$ is obtained as follows:
\begin{itemize}
    \item for each initial state $I$, if  it has incoming transitions, 
    add a fresh state with the same derivatives, and a $1$-labelled transition from it to the former initial state, 
    add a transition labelled~$n$ from the new initial state to it;
    \item add a fresh final state, called the \emph{sink}, which becomes the unique final state with no extensions;
    \item for each former final state and each of its extensions~$n$, 
    add a transition labelled~$n$ from that state to the sink.
\end{itemize}
The obtained network is denoted by $\llbracket C\rrbracket$, 
\end{definition}
