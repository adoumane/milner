...
\begin{definition}
  A \emph{network} has the same data as a chart, but instead of a single initial state, it has a list of initial states.
\end{definition}
...
\begin{example} Example of a Network with two initial states, one of them has incoming transitions, and two final states
\begin{center}
\begin{tikzpicture}[>=Latex,baseline=(p1.base),
    state/.style={circle,draw,inner sep=1.6pt},           
      final/.style={state,double,double distance=1pt},  
      lab/.style={font=\small}]
      % initial states (sans étiquettes)
      \node[state] (p1) at (0,0) {};
      \node[state] (p2) at (2,0) {};

      \node[state] (c) at (1,-1.5) {};
      % état final (sans étiquette)
      \node[final] (q1) at (0,-3) {};   
      \node[final] (q2) at (2,-3) {};
      % flèches entrantes verticales numérotées 1..n
      \draw[->] ([yshift=6mm]p1.north) -- node[left,lab] {$1$} (p1.north);
      \draw[->] ([yshift=6mm]p2.north) -- node[left,lab] {$2$} (p2.north);
      % transitions étiquetées par phi(i)
      \draw[arr] (p1) to node[left,lab, pos=0.4]  {$a$} (c);
      \draw[arr] (p2) to node[right,lab, pos=0.4] {$b$} (c);

      \draw[arr] (c) to node[left,lab, pos=0.4]  {$a$} (q1);
      \draw[arr] (c) to node[right,lab, pos=0.4] {$c$} (q2);
      % transition entrante vers p1
      \draw[->] (c) to node[above,lab, bend right] {$a$} (p1);
\end{tikzpicture}
\hspace{2cm}
\end{center}
\end{example}
...
\begin{definition} Let $E=(e_1,\dots,e_n)$ be a list of regular expressions. The \emph{network} $\mathcal{N}(E)$ is defined as follows:
  \begin{center}
  \begin{tikzpicture}[>=Latex,baseline=(p1.base),
    state/.style={circle,draw,inner sep=1.6pt},
    final/.style={state,double,double distance=1pt},
    ...
    % initial states (sans étiquettes)
    \node[state] (p1) at (0.5,0) {};
    %\node[state] (p2) at (1.2,0) {};
    \node (dots) at (2,0) {$\cdots$};
    \node[state] (pn) at (3.5,0) {};
    % état final (sans étiquette)
    \node[final] (q) at (2,-1.2) {};
    % flèches entrantes verticales numérotées 1..n
    \draw[->] ([yshift=6mm]p1.north) -- node[left,lab] {$1$} (p1.north);
    %\draw[->] ([yshift=6mm]p2.north) -- node[left,lab] {$2$} (p2.north);
    \draw[->] ([yshift=6mm]pn.north) -- node[left,lab] {$n$} (pn.north);
    % transitions étiquetées par phi(i)
    \draw[arr] (p1) to node[left,lab, pos=0.4,xshift=-2mm]  {$e_1$} (q);
    %\draw[arr] (p2) to node[left,lab, pos=0.4]  {$\varphi(2)$} (q);
    \draw[arr] (pn) to node[right,lab, pos=0.4, xshift=2mm] {$e_n$} (q);
  \end{tikzpicture}
  \end{center}
\end{definition}
...
\begin{definition}
Let $N$ be a network and let $E$ be a list of regular expressions. If $\norm{N} \twoheadrightarrow^* \mathcal{N}(E)$, we say $N$ \emph{reduces} to $E$ and write:
\[N \twoheadrightarrow^* E.\]
When $N$ is a chart, and hence $E$ reduces to a single element $e$, 
we simply write:
\[
N \twoheadrightarrow^* e.
\]
...
The image of the initial states list of $N$ by $\ell$ is a list of  expressions called its \emph{interface}. If $N$ admits a labeling with interface $E$, we write \[N\models E.\]
\end{definition}

\begin{remark} Note that for every network $N$ and list of expressions $E$ we have:
  \[N\models E\quad \text{iff}\quad [N]\models E.\]
\end{remark}
...
The rewriting rules interact well with the notion of labelability. Indeed, labelability is preserved by the inverse of the reduction relation, and as a corollary we deduce that reducibility implies labelability (Section~\ref{sec:reducible-implies-decorable}).  Labelability is also preserved under reduction (under a safety hypothesis), and a corollary of this is a form of confluence (Section~\ref{sec:confluence-of-rewriting-system}).
 ...
 \subsection{Reducible implies labelable}\label{sec:reducible-implies-decorable}
   \begin{lemma}
     Let $M$ and $N$ be two networks. We have:
     ...
     By noticing that for every list of expressions $E$, we have
     $\mathcal{N}(E)\models E$, we obtain the following proposition.
 \begin{proposition}
    For every network $N$ and list of expressions $E$, we have:
    \[ N\twoheadrightarrow^* E \quad \Longrightarrow \quad N \models E.\]~\label{prop:reducible-implies-decorable}~\\[-25pt]
 \end{proposition}
...
\subsection{Confluence of the rewriting system}\label{sec:confluence-of-rewriting-system}
 ...
 \end{remark}
  ...
  \begin{lemma}~\label{lem:stable-by-red} Let $M$ and $N$ be two networks safe for reduction. We have:
    \[  N \models E  \ \text{ and }\  N \twoheadrightarrow M\quad \Longrightarrow\quad M \models E,\] for every list of expressions $E$.
  \end{lemma}
\begin{proof}
...
\begin{proposition} Let $N$ be a network safe for reduction. We have:
 \[ N \models E\ \text{ and }\  N\twoheadrightarrow^* F\quad \Longrightarrow\quad E\equiv F,\] for every lists of expressions $E$ and $F$.
\end{proposition}
\begin{proof} 
By iterating lemma~\ref{lem:stable-by-red}, and noticing that:
 \[\mathcal{N}(F)\models E\quad \Rightarrow \quad E\equiv F.\]\vspace{-10pt}
\end{proof}

As a consequence, we show a sort of confluence for the rewriting system.
\begin{corollary} Let $N$ be a network. We have: 
  \[ N \twoheadrightarrow^* E\ \text{ and }\  N\twoheadrightarrow^* F\quad \Longrightarrow\quad E\equiv F.\]
  for every lists of expressions $E$ and $F$. 
