...
\section{Rewriting system for charts}
    ...
    \begin{center}
     \begin{tikzpicture}[node distance=1.8cm, scale=1.0, every node/.style={transform shape}]
        ...
        \end{tikzpicture}\\
    \end{center}
       \item \textbf{Loop elimination} (the circle state is not initial, nor final):\\
    ...
    \end{center}
    \end{itemize}
    In words, if in a chart, some $q_1,\dots, q_m $ have a common subset
     $S$ of derivatives, that is
    $S\subseteq D(q_i)$ for every $i\in[1,m]$. Then the rewriting rule 
    $\twoheadrightarrow_{\mathsf{f}}$:
    \begin{itemize}
    \item creates a fresh state $q$ such that $D(q)=S$,
    \item removes $S$ from $D(q_i)$ and adds $(1,q)$, for every $i\in[1,m]$.
    \end{itemize}
