...
\begin{definition}
The \emph{derivatives} $\partial(e)$ of a regular expression $e$ over $\Sigma$
are defined inductively as follows: \todo{Clarifier si $e\in \partial(e)$ ou non.}
\begin{itemize}
  \item $\partial(0) = \partial(1) = \emptyset$;
  \item $\partial(a) = \{1\}$ for $a \in \Sigma$;
  \item $\partial(e+f) = \partial(e) \cup \partial(f)$;
  \item $\partial(ef) = \partial(e)f \cup \partial(f)$;
  \item $\partial(e^*) = \partial(e)e^*$.
\end{itemize}
...
\begin{itemize}
  ...
  \item $f \sqsubseteq e \quad\Longleftrightarrow\quad
    \exists g \prec e,\; f \leq g$.
\end{itemize}
\end{definition}
...
\begin{lemma}
The relations $\prec$, $\leq$ and $\sqsubseteq$ are transitive.
\end{lemma}

\subsection{Completeness}

\begin{definition}
A regular expression $e$ is:
\begin{enumerate}
  \item \emph{complete} if
  \[
    \forall f_1, f_2 \in \partial(e),\quad f_1 \sim f_2\ \Longrightarrow\  f_1 \equiv f_2;
  \]
  \item \emph{strongly complete} if, for every regular expression $g$,
  \[
    \forall f_1, f_2 \in \partial(e),\quad f_1 g \sim f_2 g\ \Longrightarrow\  f_1 g \equiv f_2 g.
  \]
\end{enumerate}
\end{definition}
...
\begin{enumerate}
  \item for all $g, h \sqsubseteq e$, the expression $gh^*$ is strongly complete;
...
\begin{itemize} 
  \item $e \sim f$, or
  \item $g \sim h^* 0\ $ for some $\ h \sqsubseteq e\ $ or $\ h \sqsubseteq f$.
\end{itemize}
\end{proposition}
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      $h\sqsubseteq h_1$ or $h\sqsubseteq h_2$.
      Assume that $h\sqsubseteq h_1$; the other case is symmetric.
      Since $h_1\prec e$, we have $h_1\sqsubseteq e$, and transitivity
      gives $h\sqsubseteq e$. Proposition~\ref{prop:sc-closure}(1)
