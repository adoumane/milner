...
\begin{definition} Let $\Sigma$ be an alphabet. We consider the rewriting rules of Figure~\ref{fig:rewriting-rules}, 
  ...
  The \emph{normalization} of~$C$, denoted~$\norm{C}$, is the network obtained from $C$ by adding a fresh state, called the \emph{sink}, which becomes the unique final state, and adding a $1$-transition from each final state of $C$ to the sink.
  \end{definition}
